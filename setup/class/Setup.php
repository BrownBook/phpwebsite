...
class Setup
{
    ...
    public function createConfig()
    {
        ...
        if (!isset($_SESSION['configSettings']['database']) || $_SESSION['configSettings']['database'] == false) {
            ...
            if (is_file($configDir . 'config.php')) {
                ...
                $this->content[] = dgettext(
                    'core',
                    'Your configuration file was written successfully!'
                ) . '<br />';
                $this->content[] = '<a href="index.php?step=3">' . dgettext(
                    'core',
                    'Move on to Step 3'
                ) . '</a>';
            } else {
                ...
                $this->content[] = '<a href="help/permissions.' . DEFAULT_LANGUAGE . '.txt">' . dgettext(
                    'core',
                    'Permission Help'
                ) . '</a>';
            }
    ...
    public function writeConfigFile()
    {
        ...
        $config_file[] = '<?php';
        $config_file[] = sprintf(
            "define('PHPWS_SOURCE_DIR', '%s');",
            PHPWS_SOURCE_DIR
        );
        $config_file[] = sprintf(
            "define('PHPWS_HOME_DIR', '%s');",
            PHPWS_SOURCE_DIR
        );
    ...
    private function postUser()
    {
        ...
        // all is well
        $aiw = true;
        if (empty($_POST['username']) || preg_match(
            '/[^\w\.\-]/',
            $_POST['username']
        )) {
            $aiw = false;
    ...
    public function postConfig()
    {
        ...
        if ($checkConnection == 1) {
            ...
            $this->messages[] = '<a href="help/database.' . DEFAULT_LANGUAGE . '.txt" target="index">' . dgettext(
                'core',
                'Database Help'
            ) . '</a>';
            return false;
    ...
    public function createDatabase()
    {
        ...
        try {
            $db = new \PDO($this->getPDODSN(), $dbuser, $dbpass);
        } catch (\Exception $e) {
    ...
    public function testDBConnect($dsn = null)
    {
        ...
        if (empty($dsn)) {
            ...
            $dbpass = $this->getConfigSet('dbpass');
            try {
                $connection = new \PDO($this->getPDODSN(), $dbuser, $dbpass);
            } catch (\Exception $e) {
    ...
    public function databaseConfig()
    {
        ...
        $formTpl['DBPORT_DEF'] = dgettext(
            'core',
            'If your host specification requires access via a specific port, enter it here.'
        );

        $formTpl['DBNAME_DEF'] = dgettext(
            'core',
            'The database\'s name into which you are installing Canopy.'
        )
            . '<br /><i>' . dgettext(
                'core',
                'Note: if you have not made this database yet, you should do so before continuing.'
            ) . '</i>';
    ...
    public function checkSession()
    {
        ...
        if (!isset($_SESSION['session_check'])) {
            ...
            $this->content[] = sprintf(
                '<a href="help/sessions.%s.txt">%s</a>',
                DEFAULT_LANGUAGE,
                'Sessions Help'
            );
            $this->content[] = sprintf(
    ...
    public function finish()
    {
        ...
        $this->content[] = dgettext(
            'core',
            'Installation of Canopy is complete.'
        ) . '<br />';
        $this->content[] = dgettext(
            'core',
            'If you experienced any error messages, check your error.log file.'
        ) . '<br />';
        ...
        $this->content[] = '<a href="../index.php">' . dgettext(
            'core',
    ...
    public function checkServerSettings()
    {
        ...
        $test['gd']['fail'] = sprintf(
            dgettext(
                'core',
                'You need to compile the %sGD image library%s into PHP.'
            ),
            '<a href="http://www.libgd.org/Main_Page">',
            '</a>'
        );
        ...
        $test['php_version']['pass'] = version_compare(
            $phpversion,
            '5.1.0',
            '>='
        );
        $test['php_version']['fail'] = sprintf(
            dgettext(
                'core',
